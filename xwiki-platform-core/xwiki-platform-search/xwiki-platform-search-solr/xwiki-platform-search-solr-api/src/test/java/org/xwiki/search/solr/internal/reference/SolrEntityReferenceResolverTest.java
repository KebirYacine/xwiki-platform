...
public class SolrEntityReferenceResolverTest
{
    ...
    public void configure() throws Exception
    {
        ...
        solrDocument.setField(FieldUtils.DOCUMENT_PARENT_PATH, "Path.To");
        solrDocument.setField(FieldUtils.DOCUMENT_NAME, "Success");
        solrDocument.setField(FieldUtils.DOCUMENT_FINAL, false);
        ...
        EntityReferenceResolver<String> defaultStringEntityReferenceResolver =
            this.mocker.getInstance(EntityReferenceResolver.TYPE_STRING);
        WikiReference wikiReference = new WikiReference("chess");
        SpaceReference spaceReference = new SpaceReference("To", new SpaceReference("Path", wikiReference));
        when(defaultStringEntityReferenceResolver.resolve("Path.To", EntityType.SPACE, wikiReference)).thenReturn(
            spaceReference);
        spaceReference = new SpaceReference("Success", spaceReference);
        when(defaultStringEntityReferenceResolver.resolve("", EntityType.DOCUMENT, spaceReference)).thenReturn(
            new DocumentReference("WebHome", spaceReference));
